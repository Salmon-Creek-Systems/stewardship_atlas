...
class TestSharedData(SessionTestCase):
    ...
    def test_writes_through_local_are_not_written_back(self):
        # Anything written through local/ is shared-data territory, not this
        # atlas's data, so it must not land in its staging prefix. The site's
        # CSS and help HTML used to be written here by every materialize; they
        # are published from the repo now (#181, site_assets.py).
        with self.session() as session:
